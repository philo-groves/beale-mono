...
Guest Agent execution and transfer commands pass through a host-built native arm64 exec helper at `/tmp/.beale-tart-exec-v3` that closes every inherited non-stdio descriptor before executing the requested argument vector. The plugin probes descriptor pressure before each guest operation and, before saturation, recycles the launchd-managed Guest Agent service without rebooting the VM or replaying the requested command. Tart operations for one VM are serialized across that maintenance boundary. A proof runbook must call `inspect_tart_vm` immediately before its execution, then use at most one bounded `tart exec <vm> /tmp/.beale-tart-exec-v3 <artifact> ...` entry command for that phase. Repeated direct RPCs or host-side polling loops bypass recovery and recreate the upstream control-socket leak.
