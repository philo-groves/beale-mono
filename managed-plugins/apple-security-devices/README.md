...
Set `tartTransportPolicy` in the private `host-config.json` inside this plugin's data directory to `guest-agent-only` or `guest-agent-or-ssh`. The latter is the compatibility default. Tart operation and runbook inputs accept `transport: "auto"`, `"guest-agent"`, or `"ssh"`; explicit SSH is available only under `guest-agent-or-ssh` and uses the operator-configured identity, pinned known-hosts file, and bounded command runner. In `guest-agent-only` mode, SSH remains disabled. `APPLE_SECURITY_TART_TRANSPORT_POLICY` provides the equivalent process-level override.
