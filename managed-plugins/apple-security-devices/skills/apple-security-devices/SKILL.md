...
9. For Tart lifecycle, readiness, diagnostics, and transfer, use the managed Tart tools rather than building an unbounded shell wrapper around SSH, SCP, DHCP, Softnet, route repair, or a host root runner. These tools address the selected guest by VM name, apply the configured transport policy, serialize operations, close inherited Guest Agent descriptors, and recover the native helper when needed. When a host-to-guest proof genuinely requires network communication, set the Tart operation or `tart-vm` proof-cell executor `transport` to `ssh`; this uses only the operator-configured SSH identity, pinned known-hosts file, and bounded command runner. The default `auto` transport still prefers Guest Agent and falls back to that same SSH configuration. Build one stable executable on the host and let `runbook.run` materialize, stage, inspect, invoke, capture evidence, and clean it up. Keep waiting or polling inside the guest executable and never issue direct diagnostic or polling RPCs from the runbook. If the selected transport is unavailable, preserve one setup blocker and repair the configured guest or SSH readiness rather than repeatedly changing VM networking.
