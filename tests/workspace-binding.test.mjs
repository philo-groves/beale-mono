...
test("stored workspace binding prefers the Beale research subject and redacts credential references", async () => {
  ...
  try {
    ...
    insertAsset.run("asset_domain", "scope_recorded", "in_scope", "domain", "example.test", "public", "{}", "2026-01-01T00:00:01Z");
    insertAsset.run("asset_host", "scope_recorded", "in_scope", "host", "api.example.test", "public", "{}", "2026-01-01T00:00:02Z");
    insertAsset.run("asset_credential", "scope_recorded", "in_scope", "credential_ref", credentialReference, "secret", "{}", "2026-01-01T00:00:03Z");
    insertAsset.run("asset_out", "scope_recorded", "out_of_scope", "domain", "excluded.example.test", "public", "{}", "2026-01-01T00:00:04Z");
    database
    ...
    database
      ...
      .run("claude-session-two", JSON.stringify({
        resourceContext: {
          kind: "report",
  ...
  try {
    ...
    assert.deepEqual(binding.authorizedAssetIds, ["asset_domain", "asset_host"]);
    assert.deepEqual(binding.resources.map((resource) => ({
      id: resource.id,
      direction: resource.direction,
      kind: resource.kind,
      locator: resource.locator,
    })), [
      { id: "asset_domain", direction: "in_scope", kind: "domain", locator: "example.test" },
      { id: "asset_host", direction: "in_scope", kind: "other", locator: "api.example.test" },
      { id: "asset_out", direction: "out_of_scope", kind: "domain", locator: "excluded.example.test" },
    ]);
    assert.match(binding.projectNotes.join("\n"), /Parser boundary confusion/);
    assert.match(binding.projectNotes.join("\n"), /artifact_report/);
    assert.match(binding.projectNotes.join("\n"), /current revision 3/);
    assert.match(binding.projectNotes.join("\n"), /report\.revise/);
    const serialized = JSON.stringify(binding);
