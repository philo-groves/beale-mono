...
test("control stream re-acknowledges reconnect retries without applying a control twice", async () => {
  ...
  const message = {
    schemaVersion: 1,
    type: "steer",
    requestId: "shared-device-control-1",
    instruction: "Inspect the shared session once.",
  };
  input.write(`${JSON.stringify(message)}\n`);
  input.write(`${JSON.stringify(message)}\n`);

  assert.deepEqual(await controls.takeSteeringInstructions(), ["Inspect the shared session once."]);
  assert.equal(events.length, 2);
  assert.equal(events[0].replayed, undefined);
  assert.equal(events[1].replayed, true);

  input.write(`${JSON.stringify({ ...message, instruction: "Different payload." })}\n`);
  await new Promise((resolve) => setImmediate(resolve));
  assert.equal(events[2].accepted, false);
  assert.match(events[2].error, /reused with a different payload/);
  controls.close();
  input.destroy();
});
