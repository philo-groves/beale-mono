...
test("memory graph saves concise knowledge additively and corrects it by revision", async () => {
  ...
  try {
    ...
    const expectedLegacyId = `invariant_${createHash("sha256")
      .update(`${store.getContext().subjectId}:invariant:parser state crosses requests`)
      .digest("hex")
      .slice(0, 20)}`;
    ...
    assert.throws(
      () => store.save({ type: "invariant", title: "Absolute evidence", evidence: [{ kind: "code", pathBase: "repository", path: "/tmp/parser.ts", locator: {}, summary: "bad path" }] }),
      /must be relative/,
    );
    assert.throws(
    ...
    assert.throws(
      () => store.save({ type: "hypothesis", title: "Legacy candidate" }),
      /retired/,
    );
    assert.throws(
    ...
    const mitigation = store.correct(candidate.id, 1, {
      type: "mitigation",
      attributes: {},
    });
    assert.match(mitigation.id, /^mitigation_/);
    assert.equal(mitigation.type, "mitigation");
    assert.equal(store.get(candidate.id), null);
    assert.ok(store.listEdges().some((edge) => edge.fromId === mitigation.id && edge.toId === first.id));
    assert.equal(mitigation.evidence.length, 1);

    const rediscoveredEndpoint = store.correct(mitigation.id, 2, { type: "flow-endpoint", attributes: { role: "sink" } });
    assert.match(rediscoveredEndpoint.id, /^flow-endpoint_/);
    assert.equal(rediscoveredEndpoint.type, "flow-endpoint");
    assert.equal(store.get(mitigation.id), null);
    assert.ok(store.listEdges().some((edge) => edge.fromId === rediscoveredEndpoint.id && edge.toId === first.id));
...
test("general catalogs do not implicitly recall unprofiled security memory", async () => {
  ...
  let legacy;
  try {
    legacy = securityStore.save({ type: "invariant", title: "Security-only invariant" });
  } finally {
    securityStore.close();
  }
  const databasePath = getDefaultMemoryDatabasePath(workspaceRoot);
