...
The Claims plugin provides `claim.get` for either a lead or finding ID. Its default response includes the stored overview and the first 10 evidence, transition, and duplicate rows; focused `section` reads omit unrelated sections. Follow each collection's `nextOffset`, passing the returned `readRevision` as `expectedReadRevision`; restart if the claim, its duplicate relationships, or its linked runbook changed. The numeric `revision` remains the version used for edits. Evidence retains its durable reference, hash, source session, actor, and metadata; transitions retain reasons and evidence links. Retained duplicate IDs expose their canonical parent. Reads stay within the current workspace and do not change claim state. `lead.list` and `finding.list` are summary catalogs with continuation offsets; `history.search` remains the search entry point.

Claim detail collections are paginated in SQLite before crossing the worker boundary. Catalogs fetch only the selected claims and compact checklist evidence. Workspace ownership applies to evidence, transition, authorship, component, and duplicate reads.

For reproduction-grade evidence, run every active cell of the current notebook revision with `runbook.run`, supplying `sourceRevision` and `environmentFingerprint` matching the claim. Partial runs remain useful for diagnosis. Each execution retains its notebook hash, content revision, cell selection, identities, and immutable cell results; `runbook.get` with `id` and `runId` reads that snapshot in cell pages. `execution.latestSuccessfulRunId` identifies a complete successful run of the current content revision, but claim promotion additionally checks source and environment identity and successful exit status for every cell.

Independent verification references a qualifying execution run ID. The host uses the reviewing tool caller's actor/session identity, rejects claim authors reviewing their own work, and records a hash of the reviewed claim content. A distinct reviewer in the same session qualifies. Claim evidence exposes `validated` for execution/review provenance and `claimBindingHash` for recorded reviews. Changing claim content, notebook content, or source/environment identity invalidates the corresponding checks. Historical records are retained by additive migrations; older executions and reviews without this provenance cannot support new promotions.
