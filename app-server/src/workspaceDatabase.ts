// This is the compatibility implementation for Beale's pre-app-server workspace
// database. It now executes only inside the app-server process. Its public DTOs
// remain defined at the Desktop client boundary while existing databases are
// migrated in place.
// @ts-nocheck
...
import {
  ...
  resolvedBreakoutRoomStatus,
  selectRunTarget,
  serializeResearchProfile,
} from './workspaceDatabaseCompatibility.js';
...
function repositoryReferenceValueChanged(
  ...
  return JSON.stringify(rewriteRepositoryReferenceValue(value, relocations)) !== JSON.stringify(value);
}
