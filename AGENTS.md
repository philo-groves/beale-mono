...
- **Beale research runtime** (`packages/research-agent`, `packages/app-server-runtime`): the general research engine plus the shared app-server worker runtime and versioned client protocol.
- **App Server** (`app-server`): the tray-resident app-server execution host and client-neutral HTTP/WebSocket control plane.
...
- `packages/research-agent` — `@beale/research-agent`: workspace context, durable memory, tools, research profiles, and the Pi-backed agent runtime.
- `packages/app-server-runtime` — shared `@beale/app-server-runtime/protocol`, app-server worker runtime, and optional app-server client.
- `app-server` — `@beale/app-server`: the standalone tray-resident process (Electron host on Windows/macOS, headless Node otherwise) that hosts app-server workers, executes canonical operations, and publishes `~/.beale/app-server.json`.
- `tests` — app-server `node:test` suite; imports built output from `../packages/app-server-runtime/dist/` and `../packages/research-agent/dist/`.
...
- Keep terminology consistent:
  ...
  - Execution posture: Beale and app-server run with the current user's host privileges; users should launch them inside their own VM/container when OS isolation is required

## CHANGELOG.md Management

- Maintain the root `CHANGELOG.md` for product, architecture, persistence, security model, and notable UX changes. Entries are grouped under `### Beale` and `### app-server` inside `## Unreleased`.
...
- Explicit product integrations, bundled Research Kits, and managed plugins may include public vendor-specific program guidance, workflows, terminology, and identifiers that are required or useful to that feature, along with focused tests of that public behavior. This exception does not cover private or machine-derived research, current target state, findings, hypotheses, exploit-chain progress, reproduction artifacts, session commentary, or identifiers copied from a live workspace. Use synthetic inputs whenever a public integration contract does not require the real identifier.
...
- Generated files and verifier outputs are candidate artifacts until accepted into durable app-server/Beale storage.
- Confirmed vulnerability conclusions require tool, artifact, or verifier-backed evidence references.
- User-provided vulnerability claims may seed app-server hypotheses; they are not target observations by themselves.
...
- Preserve the app-server-owned research engine and global database as the canonical runtime and persistence boundaries.
- Keep product behavior, shared types, IPC contracts, protocol DTOs, and tests synchronized across app/package boundaries.
- Beale locates app-server through workspace-root discovery (`resolveAppServerWorkspaceRoot`) or the `BEALE_APP_SERVER_*` environment overrides. Do not reintroduce assumptions about a sibling checkout layout.
- Run transport and canonical operations go through the Beale app-server. Desktop and iOS are clients; they do not host the research runtime or open app-server storage.
- Protocol DTOs, constants, and decoders come from the `@beale/app-server-runtime/protocol` package; `apps/desktop/src/main/appServerProtocol.ts` is only a re-export adapter plus Beale-side policy (required capabilities, bootstrap hardening, pinned client identity). Do not restate shared DTOs locally.
- Keep model-facing tools and durable semantics in `@beale/research-agent`; app-server owns host lifecycle, storage routing, provider policy, canonical operation dispatch, and client transport. Remote-client authorization belongs to the operator-managed network boundary.
...
- `pnpm build` / `pnpm check` / `pnpm clean` — research-runtime and app-server TypeScript project references.
- `pnpm test` / `pnpm test:fast` — default monorepo gate: fast research-runtime, app-server, and Desktop unit suites.
- `pnpm test:integration` — hosted app-server runtime and real-session Desktop suites.
- `pnpm test:all` — run both test tiers.
- `pnpm test:runtime:fast` / `pnpm test:runtime:integration` / `pnpm test:runtime:all` — component-specific research-runtime tiers.
...
- `pnpm --filter @beale/desktop test:integration` exercises real app-server sessions with bounded file concurrency and requires built packages first.
