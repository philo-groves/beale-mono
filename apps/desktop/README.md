...
- app-server-driven reasoning and discovery loops
- Strong emphasis on reproducible observations, provenance, and responsible disclosure
- app-server-backed execution, memory, trace, context, and artifact visibility
...
- **Workspaces**: Local authorized research contexts with explicit ownership in app-server's global SQLite database, Beale artifacts, and references to relevant source material
...
- **Tools**: app-server tools, skills, MCP servers, and Beale-owned disclosure/export affordances
...
- app-server-owned workspace housekeeping with separate Dejunk and Dream maintenance: new research workspaces checkpoint and quarantine disposable scratch/cache files with recovery journals; legacy workspaces retain their existing housekeeping behavior
...
- List-only app-server memory catalog with search, session/workspace/subject scope and type filters, inline details, references, and textual relationships
- Workspace-scoped Jupyter-format runbooks with feature-selected setup, runtime, and cleanup cells; whole-run, inclusive range, and per-cell controls; host or managed Tart VM executors with explicit guest/root execution posture; independent content-revision, completed-run, executed-cell, and latest-status metrics; durable cell results; bounded outputs; healthy-runbook guidance; and dedicated app-server tools and Beale sidebar visibility
...
Beale uses app-server's user-global `~/.beale/memory.sqlite` database. Operational sessions, runbook metadata, and durable knowledge retain explicit workspace, subject, and research-profile ownership, so profile selection remains workspace-local without clearing or hiding other workspaces. Session metadata, transcript/trace cursor pages, collaboration state, capture metadata, and requested event detail cross the app-server boundary as separate bounded responses. Capture rows reference normalized durable event streams instead of embedding duplicate timelines, routine event appends do not re-read or rewrite prior history, and legacy aggregate documents migrate transactionally. Structural database corruption and session hash failures remain non-retryable and instruct the operator to stop writers and preserve the original database before restore or repair. Every terminal session records a structured final disposition with typed blocker dependencies and an explicit indication of whether external state is required before meaningful progress can continue. Durable knowledge is a small graph of concise typed nodes, relationships, tags, and evidence references; transcripts, task narration, and bulk outputs are not memory. Runbooks are workspace-scoped `.ipynb` artifacts for reusable procedures and proof sequences. Their whole-run, inclusive cell-range, and per-cell controls execute only in the owning live app-server session. Host and Tart cells default to a five-minute bounded runtime and retain explicit per-cell timeouts up to 30 minutes. Host cells use the normal shell-safety boundary; Tart VM cells reference a host-built workspace executable or durable artifact that app-server materializes, streams to the named guest, inspects immediately before invoking, records with guest evidence, and cleans automatically. Each cell retains its latest status, duration, exit code, and bounded output. Content revisions are reported separately from completed runs and executed cells, so execution-state persistence is not presented as authoring activity.
...
app-server protocol v1 is the transport boundary, while contract v24 identifies the executing runtime build, current claim-ledger and memory-summary schemas, file-authority workspace synchronization and reversible derived-index release, and required capabilities. Beale validates the app-server descriptor and WebSocket hello. Session persistence and derived research queries, claims, campaign projection, knowledge and artifact operations, auxiliary model jobs and provider semantics, Agent Plugins, source materialization, and Dejunk maintenance are app-server-owned operations hosted by app-server.

Beale is pre-alpha and uses append-only component-scoped migrations.

The sidebar Skills and MCP Servers views request hosted tool discovery and configuration from app-server, so persisted skill directories, selected skill ids, MCP config paths, allowlists, and default timeouts remain in app-server's `.beale/tools.json`. Every discovered MCP tool also exposes an optional `bealeTimeoutMs` control (1 second through 30 minutes) that the research agent can use for an individual slow call; Beale strips that field before provider dispatch. Legacy synchronous Desktop call sites may use the packaged app-server client, but that executable cannot host research or access app-server storage directly.

### Research Profiles

Beale uses app-server's bundled Cybersecurity profile by default. Each workspace selects Cybersecurity or Mathematics during creation, stores that selection in the workspace registry, and can use the shared global database without changing another workspace's active profile or clearing workspace lists.
...
Before each new run, Beale asks app-server to resolve and normalize the selected bundled profile, verifies its content hash, and stores the exact snapshot in that profile's database. The run references that snapshot, and continuations, capture import, historical rendering, and memory interpretation reuse it.

### Research Kits

Each workspace also selects a Research Kit during creation. General leaves resource and authorization entry manual; HackerOne imports and normalizes a public program scope; Apple Security Bounty supplies program guidance and an optional Apple OSS repository catalog; Google OSS VRP supplies Google Bug Hunters rules and the published tiered OT0/OT1 repository catalog; MSRC Windows supplies Windows Insider Preview rules and a selectable catalog of common Windows 11 apps, services, the four sandbox contexts listed for local Attack Scenario Awards, and source repositories for shipped open-source components; and Meta Bug Bounty supplies published scope examples, responsible research rules, and guidance. MSRC Windows candidates start unselected, and researchers must confirm the installed guest build, component presence, servicing eligibility, and current program terms. The MSRC Windows tab can add or remove selected catalog resources while preserving manual resources and checkout metadata; its refresh uses guidance bundled with Beale rather than fetching live program terms. Meta imports are bundled with Beale and link to the current program pages for verification. Meta Bug Bounty sessions also load the bundled Meta Skills plugin's researcher-tool guidance when the plugin is enabled; other kits do not load that skill. Kit definitions declare their compatible Research Profiles and acquisition capabilities in one shared catalog. The selection is persisted with the workspace and shown read-only in Workspace Overview because changing it after creation could invalidate the recorded scope, resources, and rules.
...
Auto-Review receives only compact host facts about recorded authorization, operator-managed execution posture, and trusted runbook execution identity when present. It classifies proofing separately but may approve a bounded exploratory proof directly; runbooks are reserved for stabilized reusable and reproduction-grade executions. If Auto-Review denies an otherwise reviewable non-policy command, the affected session replaces its steering composer with a correlated Approve Once question; the researcher can approve only that exact command or keep it blocked. A malformed reviewer response receives one bounded schema-repair retry. Policy failures and unavailable or malformed reviews remain fail-closed, while Beale retains only the sanitized failure category, phase, and attempt count.
...
Beale's default profile delegation ceiling permits the `shell`, `repository-search`, and `file-read` families and the `none`, `read`, `write`, and `process` effects. Repository search and file reads use the current user's host filesystem permissions and may read paths outside the Beale workspace; configured repositories remain context hints rather than access fences. A trusted host operator can replace the allowed built-in families with `BEALE_APP_SERVER_PROFILE_TOOL_FAMILY_CEILING_JSON` or narrow those effects with `BEALE_APP_SERVER_PROFILE_SIDE_EFFECT_CEILING_JSON`. These JSON arrays are host configuration, not profile fields. Beale-launched app-server sessions are granted the shell network side effect uniformly; network isolation and destination control must be enforced outside the application with operator-managed VM, container, firewall, proxy, or host controls.
