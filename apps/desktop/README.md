...
The sidebar Skills and MCP Servers views request hosted tool discovery and configuration from app-server, so persisted skill directories, selected skill ids, MCP config paths, allowlists, and timeouts remain in app-server's `.beale/tools.json`. Legacy synchronous Desktop call sites may use the packaged app-server client, but that executable cannot host research or access app-server storage directly.
