...
- Tart Guest Agent descriptor monitoring now samples at bounded intervals instead of probing before every operation. Failed descriptor recovery latches guest execution for the affected VM, makes readiness return immediately, and prevents repeated guest calls until a clean stop/start clears the latch.
- Host research checkpoints now reassert the current run's host-discovered `AGENTS.md` content after every local, native, or retry compaction, keeping authoritative VM identities and execution posture adjacent to the resumed working context instead of relying only on the persistent system prompt.
- New Research prompts with an explicit campaign-track field or an unambiguous mandatory track-binding directive now bind that investigation before agent startup, preventing an empty duplicate investigation and immediate self-termination.
- New Research no longer fails when optional Darwin VM setup discovery is unavailable or unsupported by a compatibility client, and the app-server client now recognizes Darwin VM setup get and update commands.
- Tart host-to-VM file copies now stream automatically up to 4 GiB by default with longer bounded transfer timeouts; callers can still choose a tighter byte limit.
- New research configurations now allow four concurrent breakout rooms by default instead of two across the runtime, Desktop, iOS, and compatibility clients.
- The managed `apple-security-devices` plugin now supports a fail-closed `guest-agent-only` Tart transport policy that never opens SSH or invokes a host command runner when Guest Agent is unavailable. Its research guidance requires Guest Agent before runbook proof execution and forbids reviving historical root-runner transport procedures.
- Tart Guest Agent commands now use a native exec helper to close all inherited non-stdio descriptors, serialize per-VM operations, and recycle the Guest Agent before descriptor saturation without rebooting the VM or replaying requested commands. Apple device guidance prepares proof execution through the managed tools and permits only one native-wrapped Tart invocation per proof phase instead of direct host-side polling.
...
- The managed `apple-security-devices` plugin now auto-reviews bounded Tart lifecycle and guest execution; permits concurrent name-bound guests and IP resolution by default; retains opt-in exclusivity and host-only Softnet networking; defaults startup to unprivileged shared NAT; falls back internally to configured pinned-host SSH and an optional private host command runner when Tart Guest Agent or direct guest routing is unavailable; returns bounded build/SIP readiness or the actual early launcher failure; and directs stalled sessions back to research instead of VM-network/runbook churn.
...
- The managed `apple-security-devices` plugin now provides bounded, verified single-file copy tools to and from named Tart VMs through Guest Agent or its private-runner SSH fallback.
...
- Desktop and Beale iOS can now attach to and steer the same active app-server session concurrently. The app-server fans one hosted worker event stream across independently authenticated clients, retains runs when one device disconnects, and exposes operator-authenticated attachment-token minting for mobile joins.
...
- Added an authenticated app-server host control plane for Desktop, Beale-iOS, and future clients to list path-free workspace summaries and read bounded canonical memory, sessions, updates, events, collaboration, and capture metadata through app-server's versioned protocol.
...
- app-server repository discovery now uses host-supplied user-global repository Resource paths instead of treating the workspace directory as a source checkout. Workspace Dejunk relocates misplaced Git repositories into the global repository store and transactionally rewrites matching Resource and artifact references; the app-server runtime contract advances to v12 with explicit clone-mode and repository-consolidation app-server capabilities.
...
- Removed the app-server CLI execution hop. The app-server now hosts session runtimes in isolated workers with direct event/control messages and executes canonical app-server operations in-process; the former `cli` workspace moved to `packages/app-server-runtime`, and its optional executable is only an app-server client.
...
- Accepted live steering is now written once at the app-server control boundary so Desktop and mobile reconcile the same canonical user message by correlated request id; Desktop also reconnects its downstream transport without finalizing a session that remains active through another device.
...
- The app-server now exposes shared, decoded `/v1` server/session lifecycle DTOs for Desktop and iOS, returns origin-relative WebSocket transports compatible with Tailscale Serve HTTPS, persists its operator token across restarts, and keeps app-server runs attached with bounded opaque-frame replay while mobile clients disconnect and reconnect.
- The repository is now a pnpm monorepo. The desktop app lives in `apps/desktop` as `@beale/desktop`; the app-server engine and shared host runtime live under `packages/`, and app-server is the sole execution host.
...
- The desktop protocol adapter now re-exports DTOs, constants, and decoders from the `@beale/app-server-runtime/protocol` package instead of restating them locally; Beale-side policy (required capabilities, hardened bootstrap parsing, pinned `beale` client identity) remains in `apps/desktop/src/main/appServerProtocol.ts`.
- The app-server now owns the single versioned session-launch mapping for Desktop, iOS, and future clients. Clients send workspace identity and logical run intent instead of host paths, storage locations, runtime arguments, plugins, provider policy, or arbitrary environments; the server resolves those values from the shared Beale registry and canonical app-server profile snapshots without receiving provider credentials.
...
- app-server now reconstructs model-visible workspace identity, authorization, scope, rules, research profile/workflow vocabulary, repository context, and report-refinement context from canonical stored state. Continuations and forks pin exact profile id/hash snapshots even after the workspace activates a newer profile.
- Built-in agent-plugin resources and capture/profile integrity checks now live with the app-server and app-server respectively. Desktop no longer derives capture paths, imports raw capture files, or carries a second plugin-resource tree.
...
- Beale now supplies stable source-revision, execution-environment, and authorized-asset identities to app-server; campaign state drives typed research momentum before trace-text heuristics and reaches session startup only as a bounded state projection.
...
- Desktop no longer synchronously spawns an app-server collaboration query for each live subagent commentary or room-status event. Queued room/member state serves the live projection, and projection failures are contained so a protocol timeout cannot crash Desktop or interrupt the app-server-owned run.
...
- Session token totals, input/output breakdowns, cache rates, and memory search/update counts now come from app-server-owned aggregates instead of Desktop event-tail subtotals or the latest root turn. app-server session migrations 6 and 7 backfill durable usage and deduplicated memory-tool activity so Desktop projection resets do not lower long-running counts.
- Desktop now imports and periodically refreshes canonical app-server session summaries for every registered workspace, so sessions created from Beale iOS appear in the Desktop sidebar without requiring Desktop to have created them first.
...
- Desktop live app-server commentary now updates only its active registry session timestamp instead of rebuilding full workspace snapshots on the Electron main thread, preventing large session stores from freezing the renderer during streamed reasoning and subagent previews.
...
- App-server goal suggestions now use the same host Codex OAuth credential bridge as research sessions instead of retrying an older app-server credential whose refresh fails.
...
- app-server session runtimes now bind durable memory writes to their canonical session ID, while session summaries recover missing membership from successful canonical memory-tool receipts so live heat and memory counts update for sessions created during the app-server refactor.
...
- Electron-hosted app-server children now run app-server in Node mode, and canonical session reads use bounded update projections for workspace ownership plus each session's latest model-usage event, preventing the 30-second process-exit timeout and avoiding redundant readers or full event-history scans on large active profiles.
- Desktop research sessions now use the app-server's typed launch contract, show concrete startup failures, retain bounded app-server exit diagnostics in the session catalog, and replace incompatible background servers through capability-aware health checks.
...
- Background pulse momentum now recognizes app-server graph mutations and recent session chains instead of legacy Beale hypothesis/finding records.
...
- Security first-touch history now covers advisories, vendor bulletins, release notes and security-content pages, fixed-version records, official and upstream source history, vendor forks and source drops, and referenced fixes, with dated no-match and deferred-source records retained alongside matches.
...
- The app-server packages now live in the shared monorepo workspace: the engine at `packages/research-agent` and the shared protocol/host runtime at `packages/app-server-runtime`, with the `node:test` suite, examples, patches, and architecture notes at the repository root.
...
- Added migration 12's durable runbook execution ledger so reproduction gates validate successful app-server executions rather than model-supplied status claims. Historical notebook-only run metadata is not promoted into the trusted ledger; rerun an existing runbook before using it to advance a finding.
...
- Session launches can now bind an existing campaign-track ID through the shared launch contract and Beale Codex bridge; the hosted runtime validates and links that exact track instead of silently creating a new investigation from the prompt.
...
- Session catalogs now read incrementally maintained token, cost, and last-message summary metrics instead of repeatedly scanning every canonical event, preventing active Desktop and iOS polling from starving live commentary on large app-server databases.
...
- app-server core migrations can adopt an existing subject-scoped Beale memory database without attempting legacy tier-column indexes or migration steps.
...
- Added protocol-v1 operations for workspace memory summaries, complete Memory Dreaming preparation and reversible state management, runbook/report document reads, and manifest-backed artifact resolution so non-Beale clients can use the same app-server-owned boundary.
- Added app-server-owned revisioned session aggregates with transactional capture import, lifecycle transitions, live-event persistence, and versioned CLI create/query operations.
- Added the exported `@beale/app-server-runtime/protocol` v1 client contract with versioned success/error envelopes and `appServer protocol describe --json` capability discovery shared by CLI and WebSocket clients. Operations enter the advertised registry only after adopting the envelope.
...
- Consolidated CLI envelopes, WebSocket messages, transport constants, factories, and validators in `@beale/app-server-runtime/protocol`; CLI operations now echo client request IDs, WebSocket protocol errors use the shared error detail DTO, and the legacy WebSocket export path remains an alias for protocol v1 compatibility.
- Centralized provider small-model defaults, profile job applicability, plugin validation, repository checkout behavior, and retained maintenance policy in app-server so clients no longer duplicate those semantics.
...
- Added atomic startup recovery for app-server-owned active sessions, marking their active attempts paused with explicit interruption metadata and a canonical recovery event so restarted clients cannot rediscover stale active state.
...
- WebSocket bearer tokens are consumed and removed from app-server's process environment before runtime tools can inherit it.
- Enforced the official Claude boundary for Anthropic: Pi executors now reject Anthropic, Pi-authenticated model registries omit it, auxiliary title and shell-review jobs always use the Claude Agent SDK, and legacy app-server Anthropic credentials are cleanup-only.
...
- Removed app-server's application-level network profiles, destination allowlists, temporal network authorization gate, and fail-closed network veto. Network intent remains reviewer-visible audit metadata, while enforceable network isolation is delegated to operator-managed system controls.
