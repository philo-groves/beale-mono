...
export function createPiAgentExecutor(
  ...
  const controlToolDescriptors = [
    ...(options.subagents === false
      ? []
      : (options.subagentRuntimeFactory ?? defaultSubagentRuntimeFactory).toolDescriptors),
  ];
  return {
          ...
          },
          {
            ...
            beforeToolCall: async (hookContext, signal) => {
              ...
              const preflight = researchTool
                ...
                : undefined;
              if (!preflight) {
                const focusKind = researchFocusToolKind(
                  toolCall.name,
                  researchTool?.descriptor.actionClasses,
