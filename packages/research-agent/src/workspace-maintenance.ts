...
export interface WorkspaceRepositoryRelocation {
  fromPath: string;
  toPath: string;
  repositoryUrl: string | null;
}

export interface WorkspaceDejunkRepositoryResult {
  summary: WorkspaceDejunkSummary;
  repositoryRelocations: WorkspaceRepositoryRelocation[];
}
...
export function runWorkspaceDejunkAndConsolidateRepositories(
  workspacePath: string,
  options: {
    repositoryStoreDirectory: string;
    repositories?: WorkspaceRepositoryCandidate[];
  }
): WorkspaceDejunkRepositoryResult {
  const summary = runWorkspaceDejunk(workspacePath);
  const repositoryRelocations = consolidateWorkspaceRepositories(
    workspacePath,
    options.repositoryStoreDirectory,
    options.repositories ?? []
  );
  return { summary, repositoryRelocations };
}
...
function consolidateWorkspaceRepositories(
  workspacePath: string,
  repositoryStoreDirectory: string,
  configuredRepositories: WorkspaceRepositoryCandidate[]
): WorkspaceRepositoryRelocation[] {
  const root = resolve(workspacePath);
  const store = resolve(repositoryStoreDirectory);
  ensureOutsideWorkspace(root, store);
  mkdirSync(store, { recursive: true });
  const configured = new Map(
    configuredRepositories
      .filter((candidate) => isWorkspaceChild(root, candidate.path))
      .map((candidate) => [resolve(candidate.path), candidate] as const)
  );
  const relocations: WorkspaceRepositoryRelocation[] = [];
  for (const repositoryPath of findWorkspaceGitRepositories(root, [...configured.keys()])) {
    const candidate = configured.get(repositoryPath);
    const repositoryUrl = normalizeSourceRepositoryUrl(
      candidate?.repositoryUrl?.trim() || readOriginRepositoryUrl(repositoryPath)
    );
    const requestedPath = repositoryUrl
      ? sourceRepositoryCheckoutPath(repositoryUrl, candidate?.ref?.trim() ?? '', store)
      : join(store, 'workspace-imports', `${safeRepositoryName(repositoryPath)}-${shortPathHash(repositoryPath)}`, 'default');
    const destinationPath = availableRepositoryDestination(requestedPath, repositoryPath);
    try {
      moveRepositoryTree(repositoryPath, destinationPath);
      relocations.push({ fromPath: repositoryPath, toPath: destinationPath, repositoryUrl: repositoryUrl || null });
    } catch {
      // Leave repositories that cannot be moved untouched. Dejunk may still complete its other safe maintenance.
    }
  }
  return relocations;
}
...
function findWorkspaceGitRepositories(workspacePath: string, configuredPaths: string[]): string[] {
  ...
  while (stack.length > 0) {
    ...
    if (directory !== workspacePath && entries.some((entry) => entry.name === '.git')) {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        // Ignore paths that disappear during traversal.
      }
    }
  }
  return [...repositories]
...
function readOriginRepositoryUrl(repositoryPath: string): string {
  ...
  let configPath = join(gitMetadataPath, 'config');
  try {
    if (lstatSync(gitMetadataPath).isFile()) {
      const pointer = readFileSync(gitMetadataPath, 'utf8').match(/^gitdir:\s*(.+)\s*$/imu)?.[1]?.trim();
      if (!pointer) return '';
      configPath = join(resolve(repositoryPath, pointer), 'config');
    }
    let readingOrigin = false;
    for (const line of readFileSync(configPath, 'utf8').split(/\r?\n/u)) {
      if (/^\s*\[/u.test(line)) {
        readingOrigin = /^\s*\[remote\s+"origin"\]\s*$/iu.test(line);
        continue;
      }
      if (!readingOrigin) continue;
      const url = line.match(/^\s*url\s*=\s*(.+?)\s*$/iu)?.[1]?.trim();
      if (url) return url;
    }
    return '';
  } catch {
...
function availableRepositoryDestination(requestedPath: string, sourcePath: string): string {
  if (!existsSync(requestedPath)) return requestedPath;
  const suffix = shortPathHash(sourcePath);
  for (let index = 1; index < 10_000; index += 1) {
    const candidate = `${requestedPath}-workspace-${suffix}${index === 1 ? '' : `-${index}`}`;
    if (!existsSync(candidate)) return candidate;
  }
  throw new Error(`Unable to choose a unique repository destination for ${basename(sourcePath)}`);
}

function moveRepositoryTree(sourcePath: string, destinationPath: string): void {
  ...
  try {
    ...
    try {
      cpSync(sourcePath, destinationPath, { recursive: true, preserveTimestamps: true, errorOnExist: true });
      rmSync(sourcePath, { recursive: true, force: false });
    } catch (copyError) {
...
function ensureOutsideWorkspace(workspacePath: string, candidatePath: string): void {
  if (candidatePath === workspacePath || isWorkspaceChild(workspacePath, candidatePath)) {
    throw new Error('The user-global repository store must be outside the workspace.');
  }
}

function isWorkspaceChild(workspacePath: string, candidatePath: string): boolean {
  const child = relative(workspacePath, resolve(candidatePath));
  return Boolean(child) && child !== '..' && !child.startsWith(`..${process.platform === 'win32' ? '\\' : '/'}`);
}

function safeRepositoryName(repositoryPath: string): string {
  return basename(repositoryPath).replace(/[^a-z0-9._-]+/giu, '-').replace(/^-+|-+$/gu, '') || 'repository';
}
