...
export type ResearchResourceKind =
  | "domain"
  | "repository"
  | "binary"
  | "service"
...
/**
 * A bounded projection of an operator-recorded scope resource. Ambient
 * resources discovered during research are stored separately and never
 * acquire authorization merely by appearing in this catalog.
 */
export interface ResearchWorkspaceResourceContext {
  id: string;
...
export interface ResearchCollaborationProviderPreference {
  ...
  enabled: boolean;
  roles: readonly ("discoverer" | "prover" | "reviewer" | "reporter")[];
}

export interface ResearchCollaborationConfig {
  mode: "solo" | "adaptive" | "always";
  subagentMode: "simple" | "advanced";
