...
export interface MaterializedSourceRepository {
  ...
  cloneMode: SourceRepositoryCloneMode;
}

export type SourceRepositoryCloneMode = 'deep' | 'shallow';
...
const SOURCE_REPOSITORY_RE = /\b(?:https?:\/\/)?(?:github\.com|gitlab\.com|[A-Za-z0-9.-]+\.googlesource\.com)\/[A-Za-z0-9_.-]+(?:\/[A-Za-z0-9_.+-]+)*(?:\.git)?(?:[/?#][^\s<>)\]]*)?/gi;
...
export function sourceRepositoryCheckoutPath(
  repositoryUrl: string,
  ref = '',
  repositoryStoreDirectory = defaultSourceRepositoryStoreDirectory()
): string {
  return join(resolve(repositoryStoreDirectory), repositorySlug(repositoryUrl), repositoryCheckoutKey(ref.trim()));
}
...
export function materializeGitRepository(
  ...
  ref: string,
  options: { cloneMode?: SourceRepositoryCloneMode; repositoryStoreDirectory?: string } = {}
  ...
  const cloneMode = options.cloneMode ?? 'deep';
  const localPath = sourceRepositoryCheckoutPath(candidate.url, cleanRef, managedRoot);
  mkdirSync(dirname(localPath), { recursive: true });
  removeStaleTemporaryCheckouts(localPath, cleanRef);

  if (existsSync(join(localPath, '.git'))) {
    ensureRepositoryCloneMode(localPath, cloneMode);
    materializeRequestedRef(localPath, cleanRef, cloneMode);
    return {
  ...
  let cloned = true;
  try {
    runGit(gitCloneArgs(candidate.url, tempPath, cloneMode));
    if (cleanRef) {
      materializeRequestedRef(tempPath, cleanRef, cloneMode);
    }
...
export async function materializeGitRepositoryAsync(
  candidate: SourceRepositoryCandidate,
  ref: string,
  options: { cloneMode?: SourceRepositoryCloneMode; signal?: AbortSignal; repositoryStoreDirectory?: string } = {}
  ...
  if (existsSync(join(localPath, '.git'))) {
    await ensureRepositoryCloneModeAsync(localPath, cloneMode, options.signal);
    await materializeRequestedRefAsync(localPath, cleanRef, cloneMode, options);
    return {
      repositoryUrl: candidate.url,
      localPath,
      cloned: false,
  ...
  let cloned = true;
  try {
    await runGitAsync(gitCloneArgs(candidate.url, tempPath, cloneMode), options.signal);
    if (cleanRef) {
      await materializeRequestedRefAsync(tempPath, cleanRef, cloneMode, options);
    }
...
export function normalizeSourceRepositoryUrl(value: string): string | null {
  ...
  if (parsed.protocol !== 'https:' || (!SOURCE_REPOSITORY_HOSTS.has(host) && !host.endsWith('.googlesource.com'))) return null;
  const allSegments = parsed.pathname.split('/').filter(Boolean);
  const stopIndexes = [allSegments.indexOf('-'), allSegments.indexOf('+')].filter((index) => index >= 0);
  ...
  const minimumPathSegments = host.endsWith('.googlesource.com') ? 1 : 2;
  if (pathSegments.length < minimumPathSegments) return null;
...
function gitCheckoutMetadata(localPath: string, requestedRef: string): Omit<MaterializedSourceRepository, 'repositoryUrl' | 'localPath' | 'cloned' | 'ref'> {
  ...
  return {
    cloneMode: gitOutput(localPath, ['rev-parse', '--is-shallow-repository']) === 'true' ? 'shallow' : 'deep',
    head,
...
function materializeRequestedRef(localPath: string, requestedRef: string, cloneMode: SourceRepositoryCloneMode): void {
  ...
  try {
    runGit(['-C', localPath, 'fetch', ...(cloneMode === 'shallow' ? ['--depth', '1'] : []), 'origin', requestedRef]);
  } catch {
    runGit(['-C', localPath, 'fetch', '--tags', ...(cloneMode === 'shallow' ? ['--depth', '1'] : []), 'origin']);
  }
...
async function materializeRequestedRefAsync(
  localPath: string,
  requestedRef: string,
  cloneMode: SourceRepositoryCloneMode,
  ...
  if (metadata.requestedRefMatchesHead === true) return;
  try {
    await runGitAsync(['-C', localPath, 'fetch', ...(cloneMode === 'shallow' ? ['--depth', '1'] : []), 'origin', requestedRef], options.signal);
  } catch {
    await runGitAsync(['-C', localPath, 'fetch', '--tags', ...(cloneMode === 'shallow' ? ['--depth', '1'] : []), 'origin'], options.signal);
  }
  const requestedCommit = gitOutput(localPath, ['rev-parse', `${requestedRef}^{commit}`]);
...
function ensureRepositoryCloneMode(localPath: string, cloneMode: SourceRepositoryCloneMode): void {
  if (cloneMode !== 'deep' || gitOutput(localPath, ['rev-parse', '--is-shallow-repository']) !== 'true') return;
  try {
    runGit(['-C', localPath, 'fetch', '--unshallow', '--no-filter', '--tags', 'origin']);
  } catch (error) {
    if (gitOutput(localPath, ['rev-parse', '--is-shallow-repository']) === 'true') throw error;
  }
}

async function ensureRepositoryCloneModeAsync(localPath: string, cloneMode: SourceRepositoryCloneMode, signal?: AbortSignal): Promise<void> {
  if (cloneMode !== 'deep' || gitOutput(localPath, ['rev-parse', '--is-shallow-repository']) !== 'true') return;
  try {
    await runGitAsync(['-C', localPath, 'fetch', '--unshallow', '--no-filter', '--tags', 'origin'], signal);
  } catch (error) {
    if (gitOutput(localPath, ['rev-parse', '--is-shallow-repository']) === 'true') throw error;
  }
}

function gitCloneArgs(repositoryUrl: string, destinationPath: string, cloneMode: SourceRepositoryCloneMode): string[] {
  return [
    '-c',
    ...
    'clone',
    ...(cloneMode === 'shallow' ? ['--depth', '1', '--filter=blob:none'] : []),
    '--',
    repositoryUrl,
    destinationPath
  ];
