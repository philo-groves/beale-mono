  ...
  const systemPrompt = [
    ...
    ...(options.hasRunbookTools ? [
      ...
      ...(profile?.agent.runbookInstructions.map((instruction) => `- ${instruction}`) ?? [
        "- Search runbooks with history.search before recording a repeatable proof, then use runbook.list or runbook.get when the full catalog or procedure is needed. Reuse or create the matching workflow runbook for reproduction-grade evidence. Keep it as the durable, human-visible execution path throughout proof development.",
