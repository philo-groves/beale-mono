...
export function createResearchSystemPrompt(
  ...
  const systemPrompt = [
    ...
    ...(options.hasTools ? [
      "Prefer repository.search for literal source discovery inside configured repositories. Use workspace.search for file-native current-workspace artifacts, scripts, and notes that may sit outside repository roots; filter by category, extension, time, and explicit raw/temporary inclusion before broad searches, then follow nextOffset. Canonical claims, memories, runbooks, reports, and investigations live in the app-server database: use their typed tools or history.search, not exported workspace files. includeCanonicalExports exists only to diagnose an explicit point-in-time export and those files may be stale. Use history.search scope=subject for compact read-only canonical research from other workspaces sharing this Subject; it does not expose their files or permit cross-workspace mutation. In multi-repository workspaces, set repository.search root to a configured path or unique root label; treat partial=true as incomplete evidence. When a raw shell search is necessary, use a narrow working directory or path and a bounded timeout.",
