...
function runbookExecutionMetrics(
  ...
  for (const row of rows) {
    ...
    if (!metrics.latest) metrics.latest = { runId, status, startedAt: requiredString(row.started_at) };
    if (status === 'succeeded' && !metrics.latestSuccessfulRunId) metrics.latestSuccessfulRunId = runId;
...
function readNodes(
  ...
  const inferredSessionNodeIds = context.sessionId
    ? inferredSessionMemoryNodeIds(database, context.sessionId)
    : new Set<string>();
    ...
    const node: Omit<HoneycrispMemoryNodeSummary, 'provenance'> = {
      ...
      sessionIds: inferredSessionNodeIds.has(id) && context.sessionId
        ? [...new Set([...(membershipSchema
            ? sessions.get(id) ?? []
            : optionalString(row.session_id) ? [requiredString(row.session_id)] : []), context.sessionId])]
        : membershipSchema
          ? sessions.get(id) ?? []
...
function reportRecording(row: SqlRow): HoneycrispReportSummary['recording'] {
  if (
    ...
    || typeof row.recording_content_hash !== 'string'
    || typeof row.recording_size_bytes !== 'number'
  ) return null;
  return {
...
function activeMemoryCatalog(snapshot: ResearchProfileSnapshot | null): ActiveMemoryCatalog | null {
  ...
  const profileHash = researchProfileHash(snapshot.profile);
  if (profileHash !== snapshot.profileHash && legacyResearchProfileHash(snapshot.profile) !== snapshot.profileHash) {
...
function inferredSessionMemoryNodeIds(database: DatabaseSync, sessionId: string): Set<string> {
  ...
  const rows = database.prepare(`
    ...
    WHERE session_id = ?
      ...
      AND COALESCE(
        json_extract(event_json, '$.payload.event.payload.toolName'),
        json_extract(event_json, '$.payload.toolName')
      ) IN ('memory.save', 'memory.correct')
      AND COALESCE(
        json_extract(event_json, '$.payload.event.payload.status'),
        json_extract(event_json, '$.payload.status')
      ) = 'complete'
  `).all(sessionId) as SqlRow[];
  return new Set(rows.flatMap((row) => typeof row.node_id === 'string' && row.node_id.trim()
    ? [row.node_id.trim()]
    : []));
}
...
function memoryVisibility(
  ...
  if (membershipSchema) {
    // A workspace summary is durable state owned by that workspace, not the
    // broader subject-recall pool. Subject memory from another workspace stays
    // available to agent retrieval and becomes visible here only after this
    // workspace records its own membership (for example by correcting or
    // reusing the node).
    return {
      sql: `EXISTS (
        SELECT 1 FROM memory_node_workspaces visible_workspace
        WHERE visible_workspace.node_id = memory_nodes.id
          AND visible_workspace.workspace_id = ?
      )`,
      params: [context.workspaceId]
    };
  }
  // Legacy rows carry their originating workspace directly. Do not infer
  // workspace ownership from a matching subject or session id.
  return { sql: 'workspace_id = ?', params: [context.workspaceId] };
}
