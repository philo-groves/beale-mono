...
export const REPORT_TRIAGE_STATUSES = ["editing", "submitted", "reviewing", "rejected", "accepted"] as const;
export type ReportTriageStatus = (typeof REPORT_TRIAGE_STATUSES)[number];
const MAX_SUBMISSION_PACKET_BYTES = 128 * 1024 * 1024;
const MAX_REPORT_RECORDING_BYTES = 512 * 1024 * 1024;
const REPORT_RECORDING_EXTENSIONS = new Set([
...
interface ReportRow {
  ...
  recording_content_hash: string | null;
...
export class ReportStore {
  ...
  public create(input: { title: string; summary: string; content: string; status?: ReportStatus; submissionPacketPath?: string }, author?: ModelAuthor): {
    ...
    try {
      ...
      this.database.prepare(`INSERT INTO app_server_reports (
        ...
        "editing", id, relativePath, entry.contentHash, entry.sizeBytes,
  ...
  public revise(input: { id: string; expectedRevision: number; content: string; summary?: string; status?: ReportStatus; submissionPacketPath?: string; recordingPath?: string }, author?: ModelAuthor): {
    report: ReportRecord;
    artifactRef: ResearchArtifactRef;
    submissionPacketArtifactRef?: ResearchArtifactRef;
    recordingArtifactRef?: ResearchArtifactRef;
  } {
    ...
    const submissionPacketPath = input.submissionPacketPath
      ? validatedPacketSource(input.submissionPacketPath, this.packetCandidateRoots)
      : null;
    const recordingPath = input.recordingPath
      ? validatedRecordingSource(input.recordingPath, this.packetCandidateRoots)
      : null;
    this.database.exec("BEGIN IMMEDIATE");
    try {
      const row = this.readRow(id);
      ...
      const recordingImport = recordingPath
        ? this.importRecording(id, row.title, recordingPath)
        : null;
      ...
      this.database.prepare(`UPDATE app_server_reports
        SET summary = ?, status = ?, content_hash = ?, size_bytes = ?,
          submission_packet_artifact_id = ?, submission_packet_filename = ?,
          submission_packet_relative_path = ?, submission_packet_content_hash = ?,
          submission_packet_size_bytes = ?, recording_artifact_id = ?, recording_filename = ?,
          recording_relative_path = ?, recording_content_hash = ?, recording_size_bytes = ?,
          revision = ?, updated_at = ?
        ...
        recording?.artifactId ?? null, recording?.filename ?? null,
        recordingImport?.entry.relativePath ?? row.recording_relative_path,
        recording?.contentHash ?? null, recording?.sizeBytes ?? null,
        revision, updatedAt,
      ...
      return {
        ...
        ...(recordingImport ? { recordingArtifactRef: recordingArtifactRef(recordingImport.entry, row.title) } : {}),
      };
    } catch (error) {
      this.database.exec("ROLLBACK");
      throw error;
  ...
  public updateTriageStatus(input: { id: string; expectedRevision: number; triageStatus: ReportTriageStatus }): ReportRecord {
    const id = requiredText(input.id, "id", 200);
    if (!Number.isSafeInteger(input.expectedRevision) || input.expectedRevision < 1) throw new Error("expectedRevision must be a positive integer.");
    validateTriageStatus(input.triageStatus);
    this.database.exec("BEGIN IMMEDIATE");
    try {
      const row = this.readRow(id);
      if (!row) throw new Error(`Report not found in this workspace: ${id}`);
      if (row.revision !== input.expectedRevision) throw new Error(`Report revision conflict for ${id}: expected ${input.expectedRevision}, found ${row.revision}.`);
      if (row.triage_status === input.triageStatus) {
        this.database.exec("COMMIT");
      ...
      this.database.exec("COMMIT");
      const updated = this.readRow(id);
      if (!updated) throw new Error(`Report disappeared after triage status update: ${id}`);
  ...
  private toRecord(row: ReportRow): ReportRecord {
    return {
      ...
      triageStatus: row.triage_status as ReportTriageStatus,
      artifactId: row.artifact_id, submissionPacket: submissionPacketFromRow(row), recording: recordingFromRow(row),
  ...
  private importRecording(reportId: string, title: string, candidatePath: string): {
    entry: ResearchStorageArtifactManifestEntry;
    filename: string;
  } {
    const sourcePath = validatedRecordingSource(candidatePath, this.packetCandidateRoots);
    const filename = basename(sourcePath);
    const relativePath = join("report-recordings", safeSegment(this.context.workspaceId), reportId, `recording${extname(sourcePath).toLowerCase()}`);
    const path = this.absolutePath(relativePath);
    mkdirSync(dirname(path), { recursive: true, mode: 0o700 });
    const temporaryPath = `${path}.${randomUUID()}.tmp`;
    copyFileSync(sourcePath, temporaryPath);
    ...
    return {
      filename,
      entry: registerResearchStorageArtifact(this.storageLayout, {
        id: `${reportId}_recording`,
        path,
        kind: "report-recording",
        purpose: `Recording attached to research report: ${title}`,
      }),
    };
  }
...
function validateTriageStatus(status: string): asserts status is ReportTriageStatus {
  if (!REPORT_TRIAGE_STATUSES.includes(status as ReportTriageStatus)) throw new Error(`Unsupported report triage status: ${status}`);
}
...
function recordingArtifactRef(entry: ResearchStorageArtifactManifestEntry, title: string): ResearchArtifactRef {
  return { id: entry.id, kind: "report-recording", uri: pathToFileURL(entry.path).href, summary: `Recording attached to research report: ${title}`, contentHash: entry.contentHash };
}
...
function recordingFromEntry(entry: ResearchStorageArtifactManifestEntry, filename: string): ReportRecording {
  return { artifactId: entry.id, filename, sizeBytes: entry.sizeBytes, contentHash: entry.contentHash };
}
function recordingFromRow(row: ReportRow): ReportRecording | null {
  if (
    !row.recording_artifact_id
    || !row.recording_filename
    || !row.recording_content_hash
    || row.recording_size_bytes === null
  ) return null;
  return {
    artifactId: row.recording_artifact_id,
    filename: row.recording_filename,
    sizeBytes: row.recording_size_bytes,
    contentHash: row.recording_content_hash,
  };
}
...
function validatedRecordingSource(value: string, allowedRoots: readonly string[]): string {
  const candidate = requiredText(value, "recordingPath", 4_096);
  if (allowedRoots.length === 0) throw new Error("Recording import is unavailable without a configured workspace root.");
  const path = realpathSync(resolve(candidate));
  if (!allowedRoots.some((root) => pathInside(root, path))) {
    throw new Error("recordingPath must resolve inside the active workspace.");
  }
  const file = statSync(path);
  if (!file.isFile()) throw new Error("recordingPath must reference a file.");
  if (!REPORT_RECORDING_EXTENSIONS.has(extname(path).toLowerCase())) {
    throw new Error("recordingPath must reference a supported audio or video file.");
  }
  if (file.size <= 0 || file.size > MAX_REPORT_RECORDING_BYTES) {
    throw new Error(`recordingPath must be between 1 byte and ${MAX_REPORT_RECORDING_BYTES} bytes.`);
  }
  return path;
}
