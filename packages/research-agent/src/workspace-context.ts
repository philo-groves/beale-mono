...
export function createResearchWorkspaceContext(
  ...
  return {
    ...
    ...(input.resources ? { resources: uniqueResources(input.resources) } : {}),
...
export function mergeResearchWorkspaceContexts(
  ...
  return {
    ...
    ...(overlay.authorizedAssetIds ?? input.base.authorizedAssetIds
      ...
      : {}),
    ...(overlay.resources ?? input.base.resources
      ? { resources: uniqueResources(overlay.resources ?? input.base.resources ?? []) }
      : {}),
...
function normalizeResources(value: unknown): ResearchWorkspaceResourceContext[] {
  return readArray(value).flatMap((candidate) => {
    if (!isRecord(candidate)) return [];
    const id = normalizedString(candidate.id);
    const locator = normalizedString(candidate.locator);
    const direction = candidate.direction;
    const kind = candidate.kind;
    if (!id || !locator || (direction !== "in_scope" && direction !== "out_of_scope") || !isResourceKind(kind)) return [];
    return [{
      id,
      direction,
      kind,
      locator,
      ...optionalStringProperty(candidate, "name"),
      ...optionalStringProperty(candidate, "sensitivity"),
      ...optionalStringProperty(candidate, "instruction"),
      source: "explicit_scope" as const,
    }];
  });
}

function uniqueResources(resources: readonly ResearchWorkspaceResourceContext[]): ResearchWorkspaceResourceContext[] {
...
function isResourceKind(value: unknown): value is ResearchWorkspaceResourceContext["kind"] {
  return value === "domain" || value === "repository" || value === "binary" || value === "service"
    || value === "tool" || value === "documentation" || value === "other";
}
