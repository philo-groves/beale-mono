...
export const DEFAULT_SECURITY_RESEARCH_PROFILE: ResearchProfile = {
  ...
  agent: {
    ...
    runbookInstructions: [
      "Search and list existing workspace runbooks before beginning proof work. Reuse or create the matching workflow runbook before the first claim-confirming experiment, keep setup, runtime, and cleanup in it behind feature tags, and retain it as the human-visible execution path for the proof.",
...
const MATHEMATICS_MEMORY_TYPE_DEFINITIONS: readonly ResearchProfileMemoryType[] = [
  {
    id: "problem",
    name: "Problem",
    pluralName: "Problems",
...
const MATHEMATICS_MEMORY_TYPES: readonly ResearchProfileMemoryType[] = MATHEMATICS_MEMORY_TYPE_DEFINITIONS.map((type) => (
  type.id === "conjecture" || type.id === "theorem" || type.id === "counterexample"
    ? { ...type, lifecycle: "retired" as const, creatable: false }
    : type
));
...
export const DEFAULT_MATHEMATICS_RESEARCH_PROFILE: ResearchProfile = {
  ...
  agent: {
    ...
    memoryInstructions: [
      ...
      "Before saving, use history.search to find the same normalized statement or construction and refine it instead of creating a differently worded duplicate.",
  ...
  claims: {
    classifications: [
      { id: "mathematics.conjecture", name: "Conjecture", pluralName: "Conjectures", description: "A precise mathematical proposition that remains proposed or under test.", defaultProjection: "lead", order: 10, icon: "sparkles" },
      { id: "mathematics.theorem", name: "Theorem", pluralName: "Theorems", description: "A mathematical claim supported by a complete proof or independent verification.", defaultProjection: "finding", order: 20, icon: "badge-check" },
      { id: "mathematics.counterexample", name: "Counterexample", pluralName: "Counterexamples", description: "A verified object or family that refutes a specified mathematical claim.", defaultProjection: "finding", order: 30, icon: "circle-x" },
    ],
  },
  workflows: [
    {
