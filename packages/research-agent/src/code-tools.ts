...
const DEFAULT_CODE_MAX_DIRECTORIES = 2_000;
const DEFAULT_CODE_MAX_ENTRIES = 50_000;
const DEFAULT_CODE_MAX_TOTAL_BYTES = 8_000_000;
...
export function createCodeIntelligenceTools(
  ...
  return [
    createCodeTool({
      ...
      maxFileBytes,
      researchSession,
      async run(action, startedAt, signal) {
    ...
    createCodeTool({
      ...
      researchSession,
      async run(action, startedAt, signal) {
        const parse = await parseActionFile(action, rootHints, maxFileBytes, signal);
...
function createCodeTool(input: {
  ...
  return {
    ...
    async execute(action, context) {
      ...
      const execution = await completeOrError(action, startedAt, () => input.run(action, startedAt, context?.signal));
      if (execution.status !== "complete") return execution;
      const firstTouches = [];
      const repositoryRoots = new Set<string>();
      for (const path of repositoryPathsFromCodeOutput(execution.output, input.rootHints)) {
        const repository = await input.researchSession.identify(path);
        if (repository) repositoryRoots.add(repository.root);
      }
      for (const root of repositoryRoots) {
        const notice = await input.researchSession.touch(root);
        if (notice) firstTouches.push(notice);
      }
      if (firstTouches.length === 0) return execution;
      const output = isRecord(execution.output)
        ? { ...execution.output, repositoryFirstTouches: firstTouches }
        : { result: execution.output, repositoryFirstTouches: firstTouches };
      return {
        ...execution,
        output,
        modelOutput: output,
        followUpActions: [
          ...execution.followUpActions,
...
function repositoryPathsFromCodeOutput(value: unknown, rootHints: readonly string[]): string[] {
  const paths = new Set<string>();
  const visit = (candidate: unknown, depth: number): void => {
    if (depth > 4) return;
    if (Array.isArray(candidate)) {
      candidate.slice(0, 100).forEach((item) => visit(item, depth + 1));
      return;
    }
    if (!isRecord(candidate)) return;
    for (const [key, nested] of Object.entries(candidate)) {
      if ((key === "resolvedPath" || key === "root") && typeof nested === "string" && isAbsolute(nested)) {
        paths.add(nested);
      } else {
  ...
  visit(value, 0);
  if (paths.size === 0) rootHints.forEach((root) => paths.add(root));
  return [...paths];
}
...
async function collectCodeFiles(
  ...
  const maxDirectories = options.maxDirectories ?? DEFAULT_CODE_MAX_DIRECTORIES;
  const maxEntries = options.maxEntries ?? DEFAULT_CODE_MAX_ENTRIES;
  const maxTotalBytes = options.maxTotalBytes ?? DEFAULT_CODE_MAX_TOTAL_BYTES;
  ...
  async function addFile(path: string): Promise<void> {
    ...
    if (!real || seenFiles.has(real) || selectedBytes + fileStat.size > maxTotalBytes) {
      return;
    }
    seenFiles.add(real);
    selectedBytes += fileStat.size;
  ...
  async function visit(path: string): Promise<void> {
    ...
    const realDirectory = await realpath(path).catch(() => undefined);
    if (!realDirectory || seenDirectories.has(realDirectory)) return;
    if (seenDirectories.size >= maxDirectories) return;
    seenDirectories.add(realDirectory);
    const entries = await opendir(realDirectory);
    for await (const entry of entries) {
      ...
      await visit(resolve(realDirectory, entry.name));
    }
  }
...
function throwIfAborted(signal: AbortSignal | undefined): void {
  ...
  throw reason instanceof Error ? reason : new Error("Code inspection was aborted.");
}
