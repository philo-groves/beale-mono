...
export interface ShellToolOptions {
  ...
  wsl?: WslShellOptions;
  windowsPowerShell?: WindowsPowerShellOptions;
}
...
function shellParameters(platform: NodeJS.Platform): NonNullable<ResearchExecutableTool["parameters"]> {
  ...
  return {
    ...
    properties: {
      command: {
        type: "string",
        description: windows
          ? "Complete PowerShell command. Set runtime to wsl to explicitly use an installed Linux distribution. Supports pipelines, chaining, redirects, and other shell syntax. Do not combine with utility or args."
          : "Complete host shell command. Supports pipelines, chaining, redirects, and other shell syntax. Do not combine with utility or args.",
      },
      utility: {
        type: "string",
      ...
      runtime: {
        type: "string",
        enum: windows ? ["host", "wsl"] : ["host"],
        description: windows
          ? "Execution runtime. Commands and direct utilities default to the Windows host; host command form uses PowerShell. Select wsl explicitly for Linux tools."
          : "Execution runtime. Commands and direct utilities run on the host.",
      },
    },
  };
}

function shellDescription(platform: NodeJS.Platform): string {
...
export function createShellTool(options: ShellToolOptions): ResearchExecutableTool {
  ...
  const parameters = shellParameters(platform);
  ...
  const windowsPowerShell = (): string | null => resolveWindowsPowerShellExecutable(
    platform,
    options.windowsPowerShell,
  );
  return {
    descriptor: {
      name: "shell.run",
      transportName: "shell_run",
      description: shellDescription(platform),
      ...
      metadata: {
        ...
        defaultCommandRuntime: platform === "win32" ? "powershell" : "host",
  ...
  return new Promise((resolvePromise) => {
    ...
    child.on("error", (error) => {
      ...
      const message = (error as NodeJS.ErrnoException).code === "ENOENT"
        ? unavailableUtilityMessage(input.utility, input.runtime, input.platform)
        : errorMessage(error);
...
function unavailableUtilityMessage(
  utility: string,
  runtime: ShellRuntime,
  platform: NodeJS.Platform,
): string {
  ...
  const host = platform === "win32" ? "Windows host" : "host";
  const windowsRecovery = platform === "win32"
    ? " On Windows, use runtime wsl for utilities installed in the workspace's Linux environment."
    : "";
  return `Shell utility ${utility} is not available on the ${host} PATH. Do not repeat the same command.${windowsRecovery}`;
}
...
function resolveShellInvocation(input: {
  ...
  if (runtime === "host") {
    ...
    if (input.requested.kind === "command") {
      ...
      const shell = input.platform === "win32" ? input.windowsPowerShell() : "/bin/sh";
      if (!shell) throw new Error("PowerShell command execution is available only from a Windows host.");
      const args = input.platform === "win32"
    ...
    if (input.platform === "win32" && isBarePowerShellUtility(input.requested.utility)) {
      const shell = input.windowsPowerShell();
      if (!shell) throw new Error("PowerShell utility execution is available only from a Windows host.");
      return {
        runtime,
        utility: shell,
...
export function resolveWindowsPowerShellExecutable(
  platform: NodeJS.Platform,
  options: WindowsPowerShellOptions | undefined = undefined,
): string | null {
  ...
  const candidates = [
    environment.ProgramFiles ? win32.join(environment.ProgramFiles, "PowerShell", "7", "pwsh.exe") : null,
    environment.ProgramW6432 ? win32.join(environment.ProgramW6432, "PowerShell", "7", "pwsh.exe") : null,
    environment.SystemRoot ? win32.join(environment.SystemRoot, "System32", "WindowsPowerShell", "v1.0", "powershell.exe") : null,
    environment.WINDIR ? win32.join(environment.WINDIR, "System32", "WindowsPowerShell", "v1.0", "powershell.exe") : null,
  ].filter((candidate): candidate is string => Boolean(candidate));
  return [...new Set(candidates)].find(pathExists) ?? "powershell.exe";
}

function isBarePowerShellUtility(utility: string): boolean {
  return !isAbsolute(utility)
    && !utility.includes("/")
    && !utility.includes("\\")
    && ["pwsh", "powershell"].includes(utilityPolicyName(utility));
}
