  ...
  private handleLine(line: string): void {
    ...
    try {
      ...
      if (message.requestId) {
        const previous = this.processedControls.get(message.requestId);
        if (previous) {
          if (previous.message !== JSON.stringify(message)) {
            throw new Error("Control requestId was reused with a different payload.");
          }
          this.onEvent({ ...previous.event, replayed: true });
          return;
        }
      }
      ...
      if (message.requestId) {
        this.processedControls.set(message.requestId, { message: JSON.stringify(message), event });
        if (this.processedControls.size > 256) {
          const oldest = this.processedControls.keys().next().value;
          if (oldest) this.processedControls.delete(oldest);
        }
      }
      this.onEvent(event);
    } catch (error) {
